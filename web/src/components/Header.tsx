...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="h-16 border-b border-slate-800 bg-slate-950/80 backdrop-blur px-4 flex items-center justify-between sticky top-0 z-30 overflow-x-auto">
      ...
      <div className="flex items-center space-x-2">
        ...
        {onChangeLayoutMode && (
          <div className="hidden 2xl:flex items-center bg-slate-900/90 border border-purple-900/40 rounded-lg p-1 space-x-1 shadow-inner">
            <button
        ...
        {onToggleAgentPanel && (
          <button
            ...
            className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg border text-xs font-mono font-semibold transition cursor-pointer ${
              isAgentPanelOpen
                ? 'bg-purple-900/90 border-purple-500 text-purple-100 shadow-[0_0_12px_rgba(168,85,247,0.35)] ring-1 ring-purple-500/50'
                : 'bg-slate-900/90 border-slate-700 text-slate-300 hover:text-white hover:border-purple-600/70 hover:bg-slate-800/80'
            }`}
            title="Toggle Autonomous EDA Agent Deck (Accessible on every tab with real-time background context)"
          >
            <Bot className={`w-3.5 h-3.5 ${isAgentPanelOpen ? 'text-purple-300 animate-pulse' : 'text-purple-400'}`} />
